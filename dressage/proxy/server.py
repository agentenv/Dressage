...
def create_app(
    *,
    ...
    if routed_experts_dtype not in {"uint8", "uint16", "int32"}:
        raise ValueError(
            "routed_experts_dtype must be 'uint8', 'uint16', or 'int32'"
        )
    ...
    def _build_step_snapshot_record(
        *,
        ...
        response_logprobs, invalid = build_snapshot_logprobs(
            step.response_logprobs,
            prompt_length=prompt_len,
            response_length=response_len,
            token_count=len(tokens),
            remote_invalid=step.all_logprobs_invalid,
        )
    ...
    def _build_concat_step_payload(
        *,
        ...
        output_logprobs, invalid = normalize_logprobs_to_length(
    ...
    async def _chat_completions_pipeline(request: Request, body: dict[str, Any]):
        ...
        async with session.request_lock:
            ...
            response_routed_experts_chunks = canonicalize_routed_experts(
                ...
                expert_id_dtype=routed_experts_dtype,
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--routed-experts-dtype",
        choices=("uint8", "uint16", "int32"),
        default="int32",
        help="Storage dtype for routed expert IDs.",
    )
